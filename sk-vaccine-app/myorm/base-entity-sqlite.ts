...
export default class BaseEntity {
  ...
  static async find<T extends BaseEntity>(
    ...
    const db: sqlite3.Database = BaseEntity.db;

    const prototype = this.prototype as EntityPrototype;
    const allAsync = promisify(db.all).bind(db);
    const result: T[] = (await allAsync(
      `SELECT * FROM ${prototype._tableName}`
    )) as T[];
  ...
  /**
   * Takes a option value of SELECT and/or WHERE. Retrieves in the SELECT format if given
   * and all if not. Finds the first instance that matches WHERE
   * @preconditions This must be called from a class that extends `BaseEntity`,
   * has one primary attribute, and the class has an `Entity` decorator. The mysql
   * database must also be initialized using the hook `useInitDataSource`. There must be a where value
   * given and it must be the condiiton you want to select one
   * @param options an object which containst the query modifiers
   * @template T The type of the entity.
   * @returns An single BaseEntity type containing the first instance of the WHERE claus
   * @throws {InvalidEntityError} If the entity prototype is not properly defined.
   */
  static async findOne<T extends BaseEntity>(
    this: { new (): T } & typeof BaseEntity,
    options: { where: { [key: string]: any }; select?: { [key: string]: any } }
  ): Promise<T | null> {
    ...
    // Default to `*` if no `select` fields are provided
    const selectClause = select && select.length > 0 ? select.join(", ") : "*";

    const queryString = `SELECT ${selectClause} FROM ${prototype._tableName} WHERE ${whereClause} LIMIT 1`;

    try {
      const allAsync = promisify(db.all).bind(db);
      const results: T[] = (await allAsync(
        `SELECT * FROM ${prototype._tableName}`
      )) as T[];
      return this.convertToObj<T>(results)[0];
    } catch (error) {
      ...
      prototype._columns!.forEach((col) => {
        ...
        if (!(col.name! in row)) {
          throw new InvalidEntityError(
            `Query result does not contain column: "${col.name}"`
          );
        }
  ...
  static async queryObjs<T extends BaseEntity>(
    ...
    const result = await allAsync(finalQuery, params);
   ...
   * Each Column where `isList: true` will be returned as a json string, convert
   * it from a json to a list (ex. Array<Obj> list = listField.json();)
   * @throws {InvalidEntityError} If the entity prototype is not properly defined.
  ...
  static async count<T extends BaseEntity>(
    ...
    const result = (await getAsync(
      `SELECT COUNT(*) as count FROM ${prototype._tableName}`
    )) as any;
    return result.count;
  }
  ...
  static async clear<T extends BaseEntity>(
    ...
    if (prototype._tableName == undefined) {
      throw new InvalidEntityError(
        `Entity prototype has undefined attribute: _tableName`
      );
    }
    const delAsync = promisify(db.run).bind(db);
    await delAsync(`DELETE FROM ${prototype._tableName}`);
    return;
  }
  ...
  private static verifyPrototype<T extends BaseEntity>(
    ...
    if (undefinedAttrs.length > 0) {
      throw new InvalidEntityError(
        `Entity prototype has undefined attribute(s): ${undefinedAttrs.join(
          ", "
        )}`
      );
    }
  ...
  async save(): Promise<void> {
    ...
    const runAsyncParams = promisify(
      (
        query: string,
        params: any[],
        callback: (err: Error | null, rows: any[]) => void
    ...
    logger;
    const { recordExists } = (await getAsync(
      `SELECT EXISTS(SELECT 1 FROM ${tableName} WHERE id = ?) as recordExists;`,
      pkValue
    )) as any;
    // INSERT
    if (!recordExists) {
    ...
    // UPDATE
      ...
      await runAsyncParams(sql, values);
    }
  }
}
