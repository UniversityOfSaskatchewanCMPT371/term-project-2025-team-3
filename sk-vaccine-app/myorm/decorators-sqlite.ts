...
export function Entity(options?: {tableName?: string, immutable?: boolean}) {
    ...
    return function (constructor: Function) {        
        ...
        if (BaseEntity.db == undefined) {    
            BaseEntity.db = new sqlite3.Database(":memory:");
        ...
        if (options.immutable) {
            // Prepare statement to get table info
            const stmt = db.prepare(`PRAGMA table_info('${prototype._tableName}');`);
    
            // Execute the query to fetch the table structure
            stmt.all((err, result) => {
                if (err) {
                ...
                // Process the result to get the table columns
                const tableColumns = result
                    ...
                    .sort((a, b) => a.name.localeCompare(b.name)); // Adjust sorting if necessary
    
                // Process the columns from the entity prototype
                const entityColumns = prototype._columns?.map((col: any) => {
                    return { 
                        name: col.name, 
                        type: col.type, 
                ...
                .sort(); // Adjust sorting if necessary
    
                // Compare table columns with entity columns and drop table if they don't match
                if (JSON.stringify(tableColumns) !== JSON.stringify(entityColumns)) {
                    db.exec(`DROP TABLE IF EXISTS ${prototype._tableName};`);
                    console.log(`Table ${prototype._tableName} dropped due to schema mismatch.`);
                }
            });
        }
        // generate sql table.
